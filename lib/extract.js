...
var Extractor = (function () {
    ...
    Extractor.prototype.addString = function (reference, string, plural, extractedComment, context) {
        ...
        if (!this.strings[string][context]) {
            ...
            return; // remove duplicated context
        }

        var item = this.strings[string][context];
        var strKey = string.replace(/(<([^>]+)>)/ig, "").replace(/[^\w\s\-]/gi, ' ').trim().replace(/[\s\-]+/g, '-');
        item.msgid = strKey.toLowerCase();
        item.msgstr.push(string);
    ...
    Extractor.prototype.extractHtml = function (filename, src, lineNumber) {
        var extractHtml = function (src, lineNumber) {
            ...
            $('*').each(function (index, n) {
                ...
                var nodeOptions = getAttr(attr);
                if(nodeOptions) {
                    try {
                        nodeOptions = JSON.parse(nodeOptions);
                    } catch(e) {
                        nodeOptions = {};
                    }
                }
                ...
                if(!nodeOptions.extract) return; // exclude this node
